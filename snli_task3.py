...
#错误，不用管，is not all you need
# def only_self_attention_trainning(train_bert_s_id,test_bert_s_id,val_bert_s_id,train_bert_mask,test_bert_mask,val_bert_mask,train_token_type,test_token_type,val_token_type,y_train, y_test,y_val,epochs,model_t,batchs):
#     train_data = TensorDataset(train_bert_s_id,train_bert_mask,train_token_type,y_train)
#     test_data = TensorDataset(test_bert_s_id,test_bert_mask,test_token_type,y_test)
#     val_data = TensorDataset(val_bert_s_id, val_bert_mask, val_token_type, y_val)
#
...
#             train_bert_s_id_t, train_bert_mask_t, train_token_type_t,y_train_t = train_bert_s_id_t.cuda(),train_bert_mask_t.cuda(),train_token_type_t.cuda(),y_train_t.cuda()
#
#             out,list_out = model(train_bert_s_id_t,train_token_type_t,train_bert_mask_t)
#             if epoch>60:
#                 for i in range(1,8):
#                     res = list_out[i]-list_out[i-1]
#                     res_max = res.max(dim = -1)[0]
#                     res_norm1 = torch.norm(res,p=1,dim=-1)
#                     res_norm = torch.sqrt(res_max*res_norm1)
#                     l_max = list_out[i-1].max(dim=-1)[0]
#                     l_norm1 = torch.norm(list_out[i],p=1,dim=-1)
#                     l_norm = torch.sqrt(l_max*l_norm1)
#                     res = res_norm/l_norm
#                     print("res ",i,": ",res.mean())
#                 print("\n")
#             loss = loss_s(out, y_train_t)
#             loss.backward()
#             # if epoch>60:
...
#                     train_bert_s_id_t, train_bert_mask_t, train_token_type_t, y_train_t = train_bert_s_id_t.cuda(), train_bert_mask_t.cuda(), train_token_type_t.cuda(), y_train_t.cuda()
#
#                     out,_ = model(train_bert_s_id_t, train_token_type_t, train_bert_mask_t)
#                     if flag == 0:
#                         total_train = out
#                         total_label_train = y_train_t
#                         flag = 1
...
if __name__=='__main__':
    sentence1, sentence2, label = read_data(25000)
    ...
    s1_len_trans,s1_softmax = len_mask_g(s1_len,max_len)
    s2_len_trans,s2_softmax = len_mask_g(s2_len,max_len)
    ...
    s1_s_test,s2_s_train,s2_s_test,train_m_mask,test_m_mask = \
        train_test_split(sentence1_id,sentence2_id, y,s1_len_trans,s2_len_trans,s1_softmax,s2_softmax,m_mask, train_size=0.7, random_state=123, shuffle=True)
