...
class CNN_e(nn.Module):
    ...
    def forward(self,data,mask,flag):
        #(batch,sentence_len,embedding)
        ...
        if flag:
            ...
            for i in range(embed.size()[0]):
                for j in range(3):
                    argmax = conved[j][i][0].argmax()
                    if argmax>mask[i][j]-1:
                        ll += 1
                    argmax = conved[j][i][1].argmax()
                    if argmax > mask[i][j]-1:
                        ll += 1
                    argmax = conved[j][i][0].argmax()
                    if argmax == mask[i][j]-1:
                        ss += 1
                        # if conved[j][i][0][argmax] == conved[j][i][0][argmax+1]:
                        #     ee += 1
                    argmax = conved[j][i][1].argmax()
                    if argmax == mask[i][j]-1:
                        ss += 1
                        # if conved[j][i][1][argmax] == conved[j][i][1][argmax+1]:
                        #     ee += 1
            print(ll)
            print(ss)
            print(ee)
...
class RNN_e(nn.Module):
    ...
    def forward(self,data,mask,flag):
        ...
        #LSTM
        # _,(rnn,_) = self.rnn(embed)
        # out = self.l(rnn).squeeze(0)
...
def cnn_embedding_train(X_train_c,y_train,X_test_c,y_test,mask_train,mask_test,vect_len,epochs,flag_g,weigth,bias_f):
    ...
    model = CNN_e(vect_len,2,flag_g,weigth,5,bias_f)
    model.to(torch.device('cuda:0'))
    ...
    for epoch in range(epochs):
        for batch in train_iter:
            ...
            out = model(train_id,mask_train_t,0)
        ...
        if epoch%4 == 0:
            ...
            # print(loss)
            if epoch > 50:
                test_acc = model.acc(total_train, total_label, 0)
            else:
                test_acc = model.acc(total_train, total_label, 0)
            flag = 0
            total_train, total_label = None, None
            for batch in train_iter:
                ...
                if epoch==60:
                    # for name, parms in model.named_parameters():
                    #     print('-->name:', name, '-->parms_requirs:', parms.requires_grad, ' -->parms_value:', parms.data)
            ...
            # print(loss)
            # if epoch > 55:
            #     train_acc = model.acc(total_train, total_label,1)
            # else:
            train_acc = model.acc(total_train, total_label,0)
...
def rnn_embedding_train(X_train_c,y_train,X_test_c,y_test,mask_train,mask_test,vect_len,epochs,flag_g,weigth):
    train_data = TensorDataset(X_train_c, y_train,mask_train)
    test_data = TensorDataset(X_test_c, y_test,mask_test)
    ...
    for epoch in range(epochs):
        ...
        if epoch%4 == 0:
            ...
            for batch in train_iter:
                ...
                if epoch==60:
                    out = model(train_id, mask_train_t, 1)
                else:
                    out = model(train_id, mask_train_t,0)
                # loss = model.loss(out, y_train_t)
                if flag == 0:
            ...
            for batch in test_iter:
                ...
                out = model(train_id,mask_test_t,0)
                # loss = model.loss(out, y_train_t)
                if flag == 0:
                    total_test = out
            ...
            # if epoch == 80:
            #     for name, parms in model.named_parameters():
            #         print('-->name:', name, '-->grad_requirs:', parms.requires_grad, ' -->grad_value:', parms.data.mean(),"  ",parms.data.std())

            # print(total.size())
            # print(loss)
            # if epoch==60:
            #     hh,hh_bias,hi_bias = None,None,None
            #     for name, parms in model.named_parameters():
            #         if name=="rnn.weight_hh_l0":
            #             hh = parms.data
            #             print(parms.data.size())
            ...
            #         print('-->name:', name, '-->grad_requirs:', parms.requires_grad, ' -->parm_value:',parms.data.size())
            #     print(rnn_out)
            #     print(F.tanh(torch.mm(rnn_out.unsqueeze(0),hh)+hh_bias+hi_bias))
...
if __name__ == '__main__':
    ...
    sentence_id = pad_sequence(sentence_id,batch_first=True,padding_value=vect_len)
    y = torch.from_numpy(y).long()
    rnn_mask = rnn_mask_g(X_len)
    cnn_mask = cnn_mask_g(X_len,max_len)
    # print(cnn_mask.size())
    # for j in range(cnn_mask.size()[0]):
    #     print(X_len[j],"   ",cnn_mask[j][0])

    # X_len = torch.tensor(X_len).long()
    X_train, X_test,y_train,y_test,mask_train_r,mask_test_r,mask_train_c,mask_test_c  = train_test_split(sentence_id,y,rnn_mask,cnn_mask,train_size=0.7,random_state=123,shuffle=True)
