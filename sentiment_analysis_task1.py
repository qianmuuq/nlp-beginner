...
class Logistics:
    ...
    def logistics_batch_type(self,feature_data, label_data,test_data,label_test):
        log_reg = LogisticRegression(multi_class='multinomial', solver='newton-cg')
        log_reg.fit(feature_data,label_data)
        train_pre = log_reg.predict(feature_data)
        test_pre = log_reg.predict(test_data)
        train_acc = self.acc(train_pre,label_data)
...
if __name__ == '__main__':
    ...
    soft = Softmax(2)
    train_s, test_s = soft.gradient_batch_type(X_train, y_train, 5, 400, 1, X_test, y_test)
    logist = Logistics()
    train_l, test_l = logist.logistics_batch_type(X_train,y_train,X_test,y_test)

    print('\t\t',"softmax\t\t","logistics")
    print('训练集\t',train_s,"\t",train_l)
    print('测试集\t',test_s,"\t",test_l)
